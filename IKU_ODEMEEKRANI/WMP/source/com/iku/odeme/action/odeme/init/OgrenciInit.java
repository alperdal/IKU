...
public class OgrenciInit extends HttpServlet {
	protected void doPost(
		...
		try 
		{
			...
			for(Iterator itr = zlist.iterator(); itr.hasNext();)
			{
				...
				joBanka.put("value",zbanka.getBankkod() );
				ja.add(joBanka);
			}
			...
			else
			{
				...
				OgrenciBean ob = new OgrenciBean();
				ob.setOgrencino(userid);
				ob.setPartnerNo(epartner);
				session.setAttribute("ogrenci",ob);
				
				joOut.put("banka",ja);
				joOut.put("muhatapno",epartner);
				joOut.put("sonuc","S");
			}
